...
@section('content_header')
    <h1>Editar período de empleo</h1>
...
@section('content')
    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ route('admin.sucursal.employee.periods.update', $period) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="row">
                    <x-input-date-custom required name="start_date" label="Fecha de ingreso" placeholder=""
                        class="col-md-3" value="{{ $period->start_date }}" />

                    <x-input-date-custom name="end_date" label="Fecha de egreso (dejar vacío si sigue activo)"
                        placeholder="" class="col-md-3" value="{{ $period->end_date }}" />

                    <div class="col-md-3">
                        <x-label value="Tipo de contrato" />
                        {{ Form::select('contract_type_id', $contractTypes, $period->contract_type_id ?? '0', ['class' => 'select2 form-control']) }}
                    </div>

                    <div class="col-md-3">
                        <x-label value="Cargo" />
                        {{ Form::select('cargo_id', $cargos, $period->cargo_id ?? '0', ['class' => 'select2 form-control']) }}
                    </div>
                </div>

                <div class="row">
                    <x-input-custom name="termination_reason" label="Motivo de egreso (opcional)" placeholder=""
                        class="col-md-6" value="{{ $period->termination_reason }}" />
                </div>
