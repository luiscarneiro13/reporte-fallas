    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ route('users.store') }}" method="POST">
                @csrf
                <div class="row">
                    <x-adminlte-input name="name" label="Nombre" placeholder="Aquí el nombre" fgroup-class="col-md-6"
                        value="{{ old('name') }}" />
                    <x-adminlte-input type="email" name="email" label="Email" placeholder="Aquí el email"
                        fgroup-class="col-md-6" value="{{ old('email') }}" />
                ...
                <div class="row">
                    <a href="{{ route('assign_role.index') }}" class="btn-sm mr-3 btn-default" type="submit"
