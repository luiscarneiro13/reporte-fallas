...
@section('content')
    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ route('admin.sucursal.divisions.store') }}" method="POST">
                @csrf
                @if ($back_url)
                ...
                <div class="row">
                    <x-adminlte-input name="name" label="Nombre" placeholder="Nombre" fgroup-class="col-md-4"
                        value="{{ old('name') }}" />
                    <x-adminlte-input name="description" label="Descripción" placeholder="Descripción"
                        fgroup-class="col-md-8" value="{{ old('description') }}" />
                </div>

                <div class="row mt-5">
                    <a href="{{ request()->back_url ?? route('admin.sucursal.divisions.index') }}"
